...
class LibrimixTrainDataset(Data.Dataset):
    ...
    def set_speech_list_and_noise_list(self):
        ...
        for i, speech in enumerate(speech_list_C):
            ...
            audio, sr = sf.read(speech)
            if len(audio) / float(sr) < self.seg_len:
                speech_list.remove(speech)
        for i, noise in enumerate(noise_list_C):
            if i % 500 == 499:
                print("processing noise no.%d ……" % i)
            audio, sr = sf.read(noise)
            if len(audio) / float(sr) < self.seg_len * 1.2:  # the factor 1.2 comes from paper LibriMix
    ...
    def get_1s1n(self, idx):
        ...
        noise, sr = sf.read(self.noise_list[idx])
        if speed != 1:
            noise = fx(noise)
