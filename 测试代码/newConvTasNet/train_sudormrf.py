...
import sys
# from model.pretrainedConvTasNet import ConvTasNet
from sudormrf.sudormrf.dnn.models.groupcomm_sudormrf_v2 import GroupCommSudoRmRf
from sudormrf.sudormrf.dnn.models.improved_sudormrf import SuDORMRF
from model.pretrained_sudormrf import get_fusion_model
# from model.DPTConvTasNet import DPT_Conv_TasNet as ConvTasNet
...
def built_solver(parameters, dirPath):
    ...
    if os.path.exists(CHECKPOINT_FILE):
        ...
        if init_epoch >= parameters['start_epoch'] and parameters['pretrained']:
            for p in net.parameters():
                p.requires_gred = True
                # optimizer.lr /= 10

        if init_epoch == parameters['start_epoch']:
            optim = optim = torch.optim.Adam(net.parameters(), lr=parameters["LR"])
    else:
...
def built_dataloader(parameters):
    ...
    train_dataset = LibriMix(parameters["2s1n_train_path"], parameters["1s1n_train_path"],
                             sample_rate=parameters["sample_rate"], seg_len=parameters["seg_len"], ratio=parameters["RATIO"])
    val_dataset = LibriMix(parameters["2s1n_val_path"], parameters["1s1n_val_path"],
                           sample_rate=parameters["sample_rate"], seg_len=parameters["seg_len"],train=False, ratio=parameters["RATIO"])
        ...
        if parameters["HALF_LR"]:
            if val_loss >= history['pre_val_loss']:
                ...
                if history['no_imprv_count'] == parameters["HALF_LR_EPOCH"]:
                    optim.param_groups[0]['lr'] = optim.param_groups[0]['lr'] / 2.0
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        # "1s1n_train_path": "/home/photon/Datasets/Libri2Mix/wav16k/single_both_mix/train-360/",
        # "1s1n_val_path": "/home/photon/Datasets/Libri2Mix/wav16k/single_both_mix/dev/",
        "1s1n_train_path": '',
        "1s1n_val_path": '',
    ...
    SAVEPATH = "error_sudo_rm_rf_pretrained"
